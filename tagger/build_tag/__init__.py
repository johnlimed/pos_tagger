...
def train_tagger(training_file, devt_file, out_file):
    count_t_v_w, count_w_v_t, count_prev_t_v_t, count_tag = __count_bi_word(training_file)
    prob_t_given_prev_t, prob_w_given_t = __count_cond_probabilities(count_t_v_w, count_w_v_t, count_prev_t_v_t,
                                                                     count_tag)
    out_dict = {
        # COUNT_T_V_W: count_t_v_w,
        # COUNT_W_V_T: count_w_v_t,
...
def __count_bi_word(training_file):
    ...
    utils.write_out_file(count_t_against_previous_t, PREV_TAG_V_TAG_COUNT_FILENAME)
    utils.write_out_file(count_tag, COUNT_TAG_FILENAME)
    return count_tag_against_word, count_word_against_tag, count_t_against_previous_t, count_tag
...
def __count_cond_probabilities(count_t_v_w, count_w_v_t, count_prev_t_v_t, count_tag):
    ...
    for tag in count_tag.keys():
        for word in count_w_v_t:
            ...
            if tag in count_w_v_t[word]:
                ...
                prob_w_given_t[word][tag] = 0.000087
        for cur_tag in count_prev_t_v_t:
            if cur_tag not in prob_t_given_prev_t:
                prob_t_given_prev_t[cur_tag] = {}
            if tag in count_prev_t_v_t[cur_tag].keys():
                prob_t_given_prev_t[cur_tag][tag] = count_prev_t_v_t[cur_tag][tag] / float(count_tag[cur_tag])
            else:
                prob_t_given_prev_t[cur_tag][tag] = 0.000087

            # for tag in count_prev_t_v_t[cur_tag]:
            #     prob_t_given_prev_t[cur_tag][tag] = count_prev_t_v_t[cur_tag][tag] / float(count_tag[cur_tag])
    ...
    #         prob_t_given_w[tag][word] = count_t_v_w[tag][word] / float(count_tag[tag])
